...
export default function MostWinsLeaderboard() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch('/api/leaderboard/most-wins');
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-3xl font-bold">Most Wins</h1>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-700">
              <tr>
                ...
                <th className="py-4 px-6 text-left">Wins</th>
              </tr>
            </thead>
            <tbody>
              {users.map((user, index) => (
                <tr key={user._id} className={`border-b border-gray-700 hover:bg-gray-700/50 ${index < 3 ? 'bg-green-900/10' : ''}`}>
                  ...
                  <td className="py-4 px-6 font-bold text-green-400">{user.wins}</td>
                </tr>
              ))}
            </tbody>
          </table>
